...
`/v1/chat/completions` 将完成状态与 Token 用量分开判断：上游发出正式 `finish`，且有正文、思考或工具调用时，即使没有 usage 也正常结束；用量未完整报告时不伪造 `usage: 0`。收到 `finish` 后立即结束下游响应并取消上游读取，不等待 HTTP 连接关闭。

监控详情记录“收到上游输出”“收到正式结束”“最后上游事件/时间”和结束原因（不记录生成内容）。错误含义：

- `zero_output`：上游明确报告输出为零。旧版也会把未报告 usage 错判为该错误；旧记录无法反推出真实终止原因。
- `upstream_incomplete`：连接在正式 `finish` 前结束，可能已有部分思考或正文；返回错误，不伪装成完整回答。需要继续检查上游服务或中间网关的连接限制。
- `empty_response`：正式结束但没有有效输出，也没有正的输出用量。
- `stream_timeout`：读空闲超时，连续收到思考数据会重置计时；不是请求总时长上限。

客户端可能对真实的断流或上游错误自动重试。延长 `CC_STREAM_IDLE_MS` 只影响空闲等待，不能修复缺失结束事件或上游主动截断。
